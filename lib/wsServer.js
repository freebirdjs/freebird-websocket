...
/*** Event Handlers                                                  ***/
/***********************************************************************/
WsServer.prototype.permitJoinHdlr = function (nc, timeLeft) { 
    var data = {
...
WsServer.prototype.netChangedHdlr = function (dev, netDelta) {
    var data = {
        address: null,
        status: null
    };
...
WsServer.prototype.statusChangedHdlr = function (dev, status) {
    this._sendInd('dev', 'statusChanged', {status: status}, dev.getId());
};

WsServer.prototype.devIncomingHdlr = function (dev) {
    var newDev = dev.dump(),
        gads = [];
...
WsServer.prototype.devLeavingHdlr = function (dev) {
    this._sendInd('dev', 'devLeaving', {id: dev.getId()}, dev.getId());
};

WsServer.prototype.gadIncomingHdlr = function (gad) {
    var newGad = gad.dump();
...
WsServer.prototype.gadLeavingHdlr = function (gad) {
    this._sendInd('gad', 'gadLeaving', {id: gad.getId()}, gad.getId());
};

WsServer.prototype.attrReportHdlr = function (gad, attr) {
    this._sendInd('gad', 'attrReport', attr, gad.getId());
};

WsServer.prototype.devAttrsChangedHdlr = function (dev, delta) {
    this._sendInd('dev', 'attrsChanged', delta, dev.getId());
};

WsServer.prototype.gadAttrsChangedHdlr = function (gad, delta) {
