...
WsServer.prototype.receiveFreebirdEvent = function (evtName, msg) {
    ...
    proving.fn(hdlr, 'handler of ' + evtName + ' not support.');
...
WsServer.prototype._reqHdlr = function (wsClient, reqMsg) {
    ...
    this._authorize(wsClient, function (err, success) {
        ...
        if (err) {
            ...
            wsApi = self._freebirdb.find('wsApi', reqMsg.subsys, reqMsg.cmd);
...
wsServerEvtHdlrs.netReady = function (wsServer, msg) {
    wsServer._sendInd('net', 'ready', msg);
};
...
wsServerEvtHdlrs.gadAttrsChanged = function (wsServer, msg) {
...
wsServerEvtHdlrs.bannedDevIncoming = function (wsServer, msg) {
    wsServer._sendInd('net', 'bannedDevIncoming', msg);
};

wsServerEvtHdlrs.bannedDevReporting = function (wsServer, msg) {
    wsServer._sendInd('net', 'bannedDevReporting', msg);
};

wsServerEvtHdlrs.bannedGadIncoming = function (wsServer, msg) {
    wsServer._sendInd('net', 'bannedGadIncoming', msg);
};

wsServerEvtHdlrs.bannedGadReporting = function (wsServer, msg) {
