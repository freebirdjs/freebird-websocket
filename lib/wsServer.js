    ...
    wsClient.on('message', function (msg) {
        ...
        if (msg.type === 'authenticate') {
            ...
            self._fb.authenticate(wsClient, msg.data, function (err, success) {
                if (success) {
                    wsClient._auth = true;
                    wsClient.send({type: 'aurhenticated'});
                    ...
                    wsClient.close(3001, new Error('Authentication failure'));
                }
            });
        } else {
            if (wsClient._auth) {
    ...
    this._fb.authorize(wsClient, function (err, success) {
        if (success) {
            ...
            if (!_.isFunction(self._fb[subsys][cmd])) {
                self._sendRsp(wsClient, reqMsg, 1, new Error(subsys + '.' + cmd + ' is not support.'));
            } else {
